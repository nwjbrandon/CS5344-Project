...
def compute_item_similarity(ratings, user_col="userId", item_col="movieId", rating_col="rating", method="cosine", use_persist=False):
    ...
    user_item_ratings = ratings.withColumnRenamed(user_col, "user").withColumnRenamed(item_col, "item").withColumnRenamed(rating_col, "rating")
    ...
    if method == "cosine":
        ...
        mean_rating = user_item_ratings.groupby(groupby_col).agg(F.mean(F.col("rating")).alias("mean_rating"))

        user_item_ratings_normed = user_item_ratings.join(mean_rating, on=groupby_col, how="inner").withColumn("rating", F.col("rating") - F.col("mean_rating")).select("user", "item", "rating")
...
def item_cosine_similarity(user_item_ratings):
    ...
    # and all other items
    other_items = user_item_ratings.select(F.col("user").alias("user_other"), F.col("item").alias("item_other"), F.col("rating").alias("rating_other"))

    # cosine similarity numerator
    # \sum_i A_i*B_i
    numerator = (
        user_item_ratings.join(other_items, on=F.col("user") == F.col("user_other"), how="inner")
        .filter(F.col("item") != F.col("item_other"))
        .groupby(["item", "item_other"])
        .agg(F.sum(F.col("rating_other") * F.col("rating")).alias("cum_prod"))
        .select(F.col("item").alias("item_n"), F.col("item_other").alias("item_other_n"), F.col("cum_prod"))
    )
    ...
    # (\sum_i A_i^2)^{1/2}
    norms = user_item_ratings.groupby("item").agg(F.sqrt(F.sum(F.col("rating") * F.col("rating"))).alias("norm")).select(F.col("item").alias("item_d"), F.col("norm"))

    # create other norms df
    norms_other = norms.select(F.col("item_d").alias("item_other_d"), F.col("norm").alias("norm_other"))

    # compute cosine similarity denominator
    # (\sum_i A_i^2)^{1/2} * (\sum_i B_i^2)^{1/2}
    denominator = (
        numerator_items.join(norms, on=F.col("item_d") == F.col("item_n"), how="inner")
    ...
    cosine_similarity = denominator.join(numerator, on=[F.col("item_d") == F.col("item_n"), F.col("item_other_d") == F.col("item_other_n")]).select(
        F.col("item_d").alias("item"), F.col("item_other_d").alias("item_other"), (F.col("cum_prod") / F.col("norm_prod")).alias("sim")
    )
