...
recommendations_for_user = user_recs.filter(col("userId") == user_id_for_recommendations).withColumn("recommendations", explode("recommendations")).select(col("userId"), col("recommendations.*"))
